...
<div style="float:left;">校園線上點餐系統-玉福</div>
...
<div style="text-align:center;margin-top:30px;">
	<div style="width:80%;height:2000px;margin:40px auto;background:white;font-size:25px;font-weight:bold;float:center;">
		<div style="text-align:left;">
			...
			<?php 
				$sqlcmd = "SELECT * FROM product_shop WHERE shop_name LIKE '玉福' AND pd_category LIKE '主食'";
			...
			<div class="grid-container" style="font-size:20px;font-weight:bold;">
			<?php 
				$sqlcmd = "SELECT * FROM product_shop WHERE shop_name LIKE '玉福' AND pd_category LIKE '青菜'";
				$rs = querydb($sqlcmd, $db_conn);
				for ($i=0; $i<count($rs); $i++) {
			?>
			<?php 
			...
			<div class="grid-container" style="font-size:20px;font-weight:bold;">
			<?php 
				$sqlcmd = "SELECT * FROM product_shop WHERE shop_name LIKE '玉福' AND pd_category LIKE '肉類'";
				$rs = querydb($sqlcmd, $db_conn);
				for ($i=0; $i<count($rs); $i++) {
			?>
			<?php 
			...
			<div class="grid-container" style="font-size:20px;font-weight:bold;">
			<?php 
				$sqlcmd = "SELECT * FROM product_shop WHERE shop_name LIKE '玉福' AND pd_category LIKE '其他'";
				$rs = querydb($sqlcmd, $db_conn);
				for ($i=0; $i<count($rs); $i++) {
			?>
			<?php 

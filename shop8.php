...
<div style="float:left;">校園線上點餐系統-哈瓦那義大利麵</div>
...
<div style="text-align:center;margin-top:30px;">
	<div style="width:80%;height:2000px;margin:40px auto;background:white;font-size:25px;font-weight:bold;float:center;">
		<div style="text-align:left;">
			<h3>&nbsp;&nbsp;&nbsp;<font color="#EB7A77">白醬</font></h3>
			<div class="grid-container" style="font-size:20px;font-weight:bold;">
			<?php 
				$sqlcmd = "SELECT * FROM product_shop WHERE shop_name LIKE '哈瓦那義大利麵' AND pd_category LIKE '白醬'";
			...
			<h3>&nbsp;&nbsp;&nbsp;<font color="#EB7A77">紅醬</font></h3>
			<div class="grid-container" style="font-size:20px;font-weight:bold;">
			<?php 
				$sqlcmd = "SELECT * FROM product_shop WHERE shop_name LIKE '哈瓦那義大利麵' AND pd_category LIKE '紅醬'";
				$rs = querydb($sqlcmd, $db_conn);
				for ($i=0; $i<count($rs); $i++) {
			?>
			<?php 
			...
			<h3>&nbsp;&nbsp;&nbsp;<font color="#EB7A77">青醬</font></h3>
			<div class="grid-container" style="font-size:20px;font-weight:bold;">
			<?php 
				$sqlcmd = "SELECT * FROM product_shop WHERE shop_name LIKE '哈瓦那義大利麵' AND pd_category LIKE '青醬'";
				$rs = querydb($sqlcmd, $db_conn);
				for ($i=0; $i<count($rs); $i++) {
			?>
			<?php 
			...
			<h3>&nbsp;&nbsp;&nbsp;<font color="#EB7A77">辣味</font></h3>
			<div class="grid-container" style="font-size:20px;font-weight:bold;">
			<?php 
				$sqlcmd = "SELECT * FROM product_shop WHERE shop_name LIKE '哈瓦那義大利麵' AND pd_category LIKE '辣味'";
				$rs = querydb($sqlcmd, $db_conn);
				for ($i=0; $i<count($rs); $i++) {
			?>
			<?php 
			...
			<h3>&nbsp;&nbsp;&nbsp;<font color="#EB7A77">焗烤</font></h3>
			<div class="grid-container" style="font-size:20px;font-weight:bold;">
			<?php 
				$sqlcmd = "SELECT * FROM product_shop8 WHERE pd_category LIKE '焗烤'";
				$rs = querydb($sqlcmd, $db_conn);
				for ($i=0; $i<count($rs); $i++) {
			?>
			<?php 
